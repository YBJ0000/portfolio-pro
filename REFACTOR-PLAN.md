...
1. 初始化 `portfolio-pro`（App Router + TS + Tailwind；Next 版本选定后写死）。
2. 从 `first-portfolio` **只抄**：`globals.css` / 字体 / 布局间距习惯、`Header` `Footer` 结构、关键页面的 Tailwind 类名与 Framer Motion 参数——**不抄** Vanta、Redis 客户端、邮件、Edge Config、Drizzle 整套。
3. 依赖安装清单与原版对比：**刻意变短**。
...
*文档版本：v3 — 修正「留言墙 / 博客评论」与 Neon 表的对应关系，删除与 §0.0 矛盾的「guestbook 默认不做」表述；补充去掉 Redis 对 API 限流的影响说明。*
